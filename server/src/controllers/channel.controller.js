...
const checkMemberPermission = async (serverId, userId, requiredRole = ["owner", "admin"]) => {
    ...
    if (!requiredRole.includes(membership.role)) {
        throw createApiError(403, `Only ${requiredRole.join(", ")} can perform this action`);
    }
...
export const createChannel = asyncHandler(async (req, res) => {
    ...
    const { name, type, topic, category, position, isPrivate, allowedRoles } = req.body;
    const io = getIO();

    // Check permissions
    await checkMemberPermission(serverId, req.user._id, ["owner", "admin", "moderator"]);
    ...
    if (existingChannel) {
        throw createApiError(400, "A channel with this name already exists in this server");
    }
...
export const getServerChannels = asyncHandler(async (req, res) => {
    ...
    const visibleChannels = channels.filter(channel => {
        ...
        return channel.allowedRoles.length === 0 ||
    ...
    sendSuccess(res, visibleChannels);
});
...
export const getChannel = asyncHandler(async (req, res) => {
    ...
    if (channel.isPrivate && !["owner", "admin"].includes(membership.role)) {
        if (channel.allowedRoles.length > 0 &&
            !channel.allowedRoles.some(roleId => membership.roles?.includes(roleId))) {
            throw createApiError(403, "You don't have access to this private channel");
        }
    }
...
export const updateChannel = asyncHandler(async (req, res) => {
    ...
    // Check permissions
    await checkMemberPermission(channel.server, req.user._id, ["owner", "admin", "moderator"]);

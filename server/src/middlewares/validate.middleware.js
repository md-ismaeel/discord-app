...
const formatZodError = (error) => {
    ...
    return issues.map((issue) => ({
        field: issue.path && issue.path.length > 0 ? issue.path.join(".") : "unknown",
...
export const validateBody = (schema) => (req, res, next) => {
    try {
        ...
        if (error?.name === "ZodError" || error instanceof ZodError) {
            ...
            return next(
                createApiError(400, "Validation failed", formattedErrors)
            );
        }
...
export const validateParams = (schema) => (req, res, next) => {
    try {
        ...
        if (error?.name === "ZodError" || error instanceof ZodError) {
            const formattedErrors = formatZodError(error);
            return next(
                createApiError(400, "Invalid parameters", formattedErrors)
            );
        }
        return next(error);
    }
...
export const validateQuery = (schema) => (req, res, next) => {
    try {
        ...
        if (error?.name === "ZodError" || error instanceof ZodError) {
            const formattedErrors = formatZodError(error);
            return next(
                createApiError(400, "Invalid query parameters", formattedErrors)
            );
        }
        return next(error);
    }
